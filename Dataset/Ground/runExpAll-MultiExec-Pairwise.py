...
fileName = 'configCombinations-Pairwise_new.csv'
totalExecutions = 3

print(check_output("mkdir testReportsMult-Pairwise-new", shell="True").decode('cp850'))
print()
...
with open(fileName) as csv_file:
    ...
    execFile = open("executionLog-Pairwise-new.txt","a")
    execTimeFile = open("executionTime-Pairwise-new.txt","a")

    currentExec = 1
    while currentExec <= totalExecutions:
        print(f"Execution:{currentExec}")
        
        print(check_output(f"mkdir testReportsMult-Pairwise-new/execution{currentExec}", shell="True").decode('cp850'))
        print()
        ...
        for k in keys:
            ...
            print(check_output(f"mkdir testReportsMult-Pairwise-new/execution{currentExec}/report{k.replace('c','')}", shell="True").decode('cp850'))
            print()
            
            print(check_output(f"cp -Rf gnd/build/reports testReportsMult-Pairwise-new/execution{currentExec}/report{k.replace('c','')}", shell="True").decode('cp850'))
            print()
