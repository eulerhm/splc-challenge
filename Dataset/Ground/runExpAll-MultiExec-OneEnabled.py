...
fileName = 'configCombinations-OneEnabled.csv'
totalExecutions = 3

print(check_output("mkdir testReportsMult-OneEnabled", shell="True").decode('cp850'))
print()
...
with open(fileName) as csv_file:
    ...
    execFile = open("executionLog-OneEnabled.txt","a")
    execTimeFile = open("executionTime-OneEnabled.txt","a")

    currentExec = 1
    while currentExec <= totalExecutions:
        print(f"Execution:{currentExec}")
        
        print(check_output(f"mkdir testReportsMult-OneEnabled/execution{currentExec}", shell="True").decode('cp850'))
        print()
        ...
        for k in keys:
            ...
            print(check_output(f"mkdir testReportsMult-OneEnabled/execution{currentExec}/report{k.replace('c','')}", shell="True").decode('cp850'))
            print()
            
            print(check_output(f"cp -Rf gnd/build/reports testReportsMult-OneEnabled/execution{currentExec}/report{k.replace('c','')}", shell="True").decode('cp850'))
            print()
