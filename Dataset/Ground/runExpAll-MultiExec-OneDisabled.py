...
print(check_output("mkdir testReportsMult-OneDisabled", shell="True").decode('cp850'))
print()
...
with open(fileName) as csv_file:
    ...
    execFile = open("executionLog-OneDisabled.txt","a")
    ...
    while currentExec <= totalExecutions:
        ...
        print(check_output(f"mkdir testReportsMult-OneDisabled/execution{currentExec}", shell="True").decode('cp850'))
        print()
        ...
        for k in keys:
            ...
            print(check_output(f"mkdir testReportsMult-OneDisabled/execution{currentExec}/report{k.replace('c','')}", shell="True").decode('cp850'))
            print()
            
            print(check_output(f"cp -Rf gnd/build/reports testReportsMult-OneDisabled/execution{currentExec}/report{k.replace('c','')}", shell="True").decode('cp850'))
            print()
