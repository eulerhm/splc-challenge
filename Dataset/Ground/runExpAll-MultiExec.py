...
fileName = 'configCombinations-Random-NEW_R_05.csv'
totalExecutions = 3

print(check_output("mkdir testReportsMult-Random-NEW-R5", shell="True").decode('cp850'))
print()
...
with open(fileName) as csv_file:
    ...
    execFile = open("executionLog-Random-NEW-R5.txt","a")
    execTimeFile = open("executionTime-Random-NEW-R5.txt","a")

    currentExec = 1
    while currentExec <= totalExecutions:
        print(f"Execution:{currentExec}")
        
        print(check_output(f"mkdir testReportsMult-Random-NEW-R5/execution{currentExec}", shell="True").decode('cp850'))
        print()
        ...
        for k in keys:
            ...
            print(check_output(f"mkdir testReportsMult-Random-NEW-R5/execution{currentExec}/report{k.replace('c','')}", shell="True").decode('cp850'))
            print()
            
            print(check_output(f"cp -Rf gnd/build/reports testReportsMult-Random-NEW-R5/execution{currentExec}/report{k.replace('c','')}", shell="True").decode('cp850'))
            print()
